...
# 1. Delete all TXT records for @ and x-transip-mail-auth
echo "Cleaning up DNS..."
curl -sS -X DELETE \
    -H "Authorization: Bearer $TOKEN" \
    -H "Content-Type: application/json" \
    --data '{"dnsEntry": {"name": "@", "expire": 300, "type": "TXT", "content": "v=spf1 include:_spf.google.com include:_spf.transip.email ~all"}}' \
...
# 2. Restore original SPF
echo "Restoring original SPF..."
curl -sS -X POST \
    -H "Authorization: Bearer $TOKEN" \
    -H "Content-Type: application/json" \
    --data "{\"dnsEntry\": {\"name\": \"@\", \"expire\": 300, \"type\": \"TXT\", \"content\": \"$ORIGINAL_SPF\"}}" \
    "$API_URL/domains/$DOMAIN/dns"
