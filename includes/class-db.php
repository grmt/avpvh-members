...
class AVPVH_DB {
    ...
    private static function migrate_to_relationships_table(): void {
        ...
        // alone got May Hasendonckx and Fenna/Dirk's actual parents wrong):
        //   Iris Zwart        -> vriendin van Olaf Boekholt
        //   May Hasendonckx   -> vrouw van Gerrit Hasendonckx (=May Verhoeven)
        //   Sylvia Soulier    -> ouder van Dean Berendsen
        //   Henk & Mariska    -> ouder van Fenna Lip AND Dirk Lip (siblings
        //                        via shared parents, not stored directly)
        //   Henk Lip          -> man van Mariska Hoogendijk
        // Henk Lip (not a club member) was created separately as a
        // 'visitor' before this migration ran. Bram Keijers/Jaap Keijers'
        ...
        $manual_map = [
            ...
            32  => [128, 'vriendin'],  // Olaf Boekholt      -> Iris Zwart
            51  => [130, 'vrouw'],     // Gerrit Hasendonckx -> May Hasendonckx
            132 => [80,  'ouder'],     // Dean Berendsen     -> Sylvia Soulier
            127 => [134, 'ouder'],     // Fenna Lip          -> Henk Lip
            118 => [134, 'ouder'],     // Dirk Lip           -> Henk Lip
        ];
        $manual_map_extra = [
            127 => 54, // Fenna Lip -> Mariska Hoogendijk
            118 => 54, // Dirk Lip  -> Mariska Hoogendijk
        ];
        ...
        // in the old avm_partners table; Henk/Mariska never had a row).
        $henk = $wpdb->get_var("SELECT id FROM {$wpdb->prefix}avm_members WHERE lldap_user_id = 'henk.lip'");
        if ($henk) {
